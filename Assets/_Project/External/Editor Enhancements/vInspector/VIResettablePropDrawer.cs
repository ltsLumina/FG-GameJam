        ...
        void buttonInteraction()
        {
            ...
            if (!parentsWithDefaultValues.ContainsKey(parent.GetType()))
                ...
                if (parent is MonoBehaviour)
                    ...
                    {
                        var go =
 EditorUtility.CreateGameObjectWithHideFlags("Dummy object for fetching default variable values for vInspector's resettable variables feature", HideFlags.HideAndDontSave, parent.GetType());
    ...
    public static Dictionary<Type, object> parentsWithDefaultValues = new Dictionary<Type, object>();
    public static List<Type> scriptTypesWithVInspector = new List<Type>();

    public object GetParent(SerializedProperty prop)
    {
        if (prop.serializedObject.targetObject.GetType() == (fieldInfo as MemberInfo).ReflectedType) return prop.serializedObject.targetObject;
    ...
    object MoveToNext(object container, PropertyPathComponent component)
    {
        ...
        else return GetMemberValue(container, component.propertyName);
    }
