...
public class SoundDataCollection : ScriptableObject
{
    ...
    public void Init()
    {
        foreach (SoundData soundData in sounds) { soundsDictionary.Add(soundData.Tag, soundData); }
    }
    ...
    public bool CreateSound(AudioClip[] clips, string tag, CompressionPreset compressionPreset, bool forceToMono, out string result)
    {
        ...
        for (int i = 0; i < sounds.Length - 1; i++) { sounds[i] = previousSounds[i]; }
    ...
    public void RemoveAll() { sounds = Array.Empty<SoundData>(); }
}
}
