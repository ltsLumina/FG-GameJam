...
public class MusicDataCollection : ScriptableObject
{
    ...
    public void Init()
    {
        foreach (SoundData soundData in musicTracks) { musicTracksDictionary.Add(soundData.Tag, soundData); }
    }
    ...
    public bool CreateMusicTrack(AudioClip[] clips, string tag, CompressionPreset compressionPreset, bool forceToMono, out string result)
    {
        ...
        for (int i = 0; i < musicTracks.Length - 1; i++) { musicTracks[i] = previousSounds[i]; }
    ...
    public void RemoveAll() { musicTracks = Array.Empty<SoundData>(); }
}
}
