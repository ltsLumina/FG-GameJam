...
public partial class AudioToolsLibrary
{
    ...
    private void Init()
    {
        ...
        {
            Debug.LogError("You're trying to initialize a Sound, Music, Playlist, Dynamic Music or Output in its declaration. " + "Please, move the initialization to Start or Awake methods\n\n" + e);
            return;
        }

        if (SoundDataCollection == null || MusicDataCollection == null)
        {
            Debug.LogError("Audio resources haven't found. " + "Open [Tools > Audio System > Audio Creator] to solve this problem.");
            return;
        }
