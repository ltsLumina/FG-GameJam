...
public partial class Playlist // Public Methods
{
    ...
    public Playlist(Track[] playlistTracks) { SetPlaylist(playlistTracks); }
    ...
    public Playlist(string[] playlistTags) { SetPlaylist(playlistTags); }
    ...
    {
        playlist.Clear();
        foreach (string tag in playlistTags) playlist.Enqueue(AudioManager.GetTrack(tag));
        return this;
    }
    ...
    {
        playlist.Clear();
        foreach (Track track in playlistTracks) playlist.Enqueue(AudioManager.GetTrack(track.ToString()));
        return this;
    }
    ...
    public void Play()
    {
        ...
        sourcePoolElement.MarkAsPlaylist().SetVolume(volume, hearDistance).SetPitch(pitch).SetLoop(loop).SetPlaylist(playlist).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound(spatialSound).SetFadeIn
            (fadeInTime).SetFadeOut(fadeOutTime).SetId(id).SetOutput(output).OnPlay(onPlay).OnComplete(onComplete).OnLoopCycleComplete(onLoopCycleComplete).OnNextTrackStart(onNextTrackStart).OnPause
            (onPause).OnPauseComplete(onPauseComplete).OnResume(onResume).PlayPlaylist(fadeInTime);
    }
