...
public partial class Music // Public Methods
{
    ...
    public void Play(float fadeInTime = 0)
    {
        ...
        sourcePoolElement.SetVolume(volume, hearDistance).SetPitch(pitch).SetLoop(loop).SetClip(clip).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound(spatialSound).SetFadeOut(fadeOutTime).SetId
            (id).SetOutput(output).OnPlay(onPlay).OnComplete(onComplete).OnLoopCycleComplete(onLoopCycleComplete).OnPause(onPause).OnPauseComplete(onPauseComplete).OnResume(onResume).Play(fadeInTime);
    }
