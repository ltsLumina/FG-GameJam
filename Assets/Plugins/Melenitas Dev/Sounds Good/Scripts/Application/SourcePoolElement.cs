...
public partial class SourcePoolElement // Internal Methods
{
    ...
    internal SourcePoolElement SetVolume(float volume, Vector2 hearDistance, float lerpTime = 0)
    {
        ...
        if (currentState == SourceState.Paused || currentState == SourceState.Stopping || currentState == SourceState.ChangingTrack)
        {
            Debug.LogWarning($"There's a volume fade out taking place at this moment, " + "so volume won't change right now, but on the next fade in it will go " + $"up until the new volume of {volume}");
            return this;
        }

        if (lerpTime <= 0) source.volume = volume;
        else { lerpVolumeCor = StartCoroutine(LerpVolume(volume, lerpTime)); }
    ...
    internal void Play(float fadeInTime = 0)
    {
        ...
        if (fadeInTime > 0)
        {
            ...
            source.volume = 0;
            lerpVolumeCor = StartCoroutine(LerpVolume(volume, fadeInTime, (() => ChangeState(SourceState.Playing))));
        }
    }
    ...
    internal void PlayPlaylist(float fadeInTime)
    {
        ...
        if (!validClips)
        {
            Debug.LogError("There are invalid audio clips in playlist, make sure you have " + "initialized it in a method (not in the declaration)");
            return;
        }
...
public partial class SourcePoolElement // Private Methods
{
    ...
    private void HandlePlaylistPlaying()
    {
        ...
        if (loop)
        {
            if (CurrentLoopCycleTime >= playlistDuration - fadeOutTime) { CompleteLoopCycle(); }
        }
        ...
        if (PlayingTime >= playingTimeForNextSong - fadeOutTime && !changingTrack)
        {
            ...
            if (fadeOutTime > 0) lerpVolumeCor = StartCoroutine(LerpVolume(0, fadeOutTime, () => PlayNextSong()));
            else PlayNextSong();
        }
    }
    ...
    private void HandleSoundStop()
    {
        ...
        if (fadeOutTime > 0)
        {
            if (PlayingTime >= CurrentClipDuration - 0.05f - fadeOutTime) { Stop(fadeOutTime); }
        }
    ...
    private void HandlePlaylistStop()
    {
        ...
        if (fadeOutTime > 0)
        {
            if (CurrentLoopCycleTime >= CurrentClipDuration - 0.05f - fadeOutTime) { Stop(fadeOutTime); }
        }
    ...
    private void PlayNextSong(bool firstTrack = false)
    {
        ...
        {
            ChangeState(SourceState.FadingIn);
            fadeInOnChangeTrackCor = StartCoroutine(LerpVolume(volume, fadeInTime, () => ChangeState(SourceState.Playing)));
        }
        else
        {
            ChangeState(SourceState.Playing);
