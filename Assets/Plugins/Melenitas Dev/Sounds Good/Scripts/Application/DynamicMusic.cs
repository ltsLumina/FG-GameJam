...
public partial class DynamicMusic // Fields
{
    ...
    private Dictionary<string, SourcePoolElement> sourcePoolElementDictionary = new Dictionary<string, SourcePoolElement>();
...
public partial class DynamicMusic // Public Methods
{
    ...
    public DynamicMusic SetTrackVolume(Track track, float volume) { return SetTrackVolume(track.ToString(), volume); }
    ...
    public DynamicMusic SetTrackVolume(Track track, float volume, Vector2 hearDistance) { return SetTrackVolume(track.ToString(), volume, hearDistance); }
    ...
    /// <param name="newVolume">New volume: min 0, Max 1.</param>
    /// <param name="lerpTime">Time to lerp current to new volume.</param>
    public void ChangeTrackVolume(Track track, float newVolume, float lerpTime = 0) { ChangeTrackVolume(track.ToString(), newVolume, lerpTime); }
    ...
    public void Play(float fadeInTime = 0)
    {
        ...
        for (int i = 0; i < sourcePoolElementDictionary.Count; i++)
        {
            ...
            if (i == 0)
            {
                ...
                referenceSourcePoolElement.SetVolume(volumeDictionary[tag], hearDistanceDictionary[tag]).SetPitch(pitch).SetLoop(loop).SetClip(clips[i]).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound
                    (spatialSound).SetFadeOut(fadeOutTime).SetId(id).SetOutput(output).OnPlay(onPlay).OnComplete(onComplete).OnLoopCycleComplete(onLoopCycleComplete).OnPause(onPause).OnPauseComplete
                    (onPauseComplete).OnResume(onResume).Play(fadeInTime);

                continue;
            }

            newSourcePoolElement.SetVolume(volumeDictionary[tag], hearDistanceDictionary[tag]).SetPitch(pitch).SetLoop(loop).SetClip(clips[i]).SetPosition(position).SetFollowTarget(followTarget).SetSpatialSound
                (spatialSound).SetFadeOut(fadeOutTime).SetId(id).SetOutput(output).Play(fadeInTime);
        }
    }
    ...
    public void Pause(float fadeOutTime = 0)
    {
        ...
        foreach (var source in sourcePoolElementDictionary.Values) { source.Pause(fadeOutTime); }
    }
    ...
    public void Resume(float fadeInTime = 0)
    {
        ...
        foreach (var source in sourcePoolElementDictionary.Values) { source.Resume(fadeInTime); }
    }
    ...
    public void Stop(float fadeOutTime = 0)
    {
        ...
        for (int i = 0; i < sourcePoolElementDictionary.Count; i++)
        {
            ...
            {
                sourcePoolElementDictionary[tag].Stop
                (fadeOutTime, () =>
                {
