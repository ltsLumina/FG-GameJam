...
public class AudioCollectionEditorWindow : EditorWindow
{
    ...
    private SoundData[] currentDataCollection => inSoundSect ? soundDataCollection.Sounds : musicDataCollection.MusicTracks;

    private Dictionary<SoundData, bool> currentTemplateDataCollection => inSoundSect ? templateSoundDataCollection : templateMusicDataCollection;
    ...
    void OnEnable()
    {
        ...
        foreach (SoundData soundData in soundDataCollection.Sounds)
        {
            ...
            templateSoundDataCollection.Add(new SoundData(soundData.Tag, clips, soundData.CompressionPreset, soundData.ForceToMono), true);
        }
        ...
        foreach (SoundData soundData in musicDataCollection.MusicTracks)
        {
            AudioClip[] clips = new AudioClip[soundData.Clips.Length];
            for (int i = 0; i < clips.Length; i++) clips[i] = soundData.Clips[i];
            templateMusicDataCollection.Add(new SoundData(soundData.Tag, clips, soundData.CompressionPreset, soundData.ForceToMono), true);
        }
    }
    ...
    public static void ShowWindow()
    {
        ...
        AudioCollectionEditorWindow window = (AudioCollectionEditorWindow) GetWindow(typeof(AudioCollectionEditorWindow));
        window.Show();
    ...
    private void Search()
    {
        ...
        {
            if (GUILayout.Button("Clean", EditorStyles.miniButton, GUILayout.Width(60))) { searchAudioClip = null; }
        }
        ...
        foreach (SoundData soundData in currentTemplateDataCollection.Keys)
        {
            if (!string.IsNullOrEmpty(searchTag) && searchAudioClip == null) newShowValuesCollection.Add(soundData, soundData.Tag.Contains(searchTag.ToLower()));
                ...
                newShowValuesCollection.Add(soundData, soundData.Tag.Contains(searchTag.ToLower()) && soundData.Clips.Any(clip => clip == searchAudioClip));
