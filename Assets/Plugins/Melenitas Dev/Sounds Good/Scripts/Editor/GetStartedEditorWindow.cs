...
public class GetStartedEditorWindow : EditorWindow
{
    void OnGUI()
    {
        ...
        string welcomeText = "the audio manager you've always dreamed of. " + "If you need any help getting started, you can refer to the documentation available in both English and Spanish.";
        ...
        if (GUILayout.Button("Create Dependencies", GUILayout.Height(50))) { CreateDependencies(); }
        ...
        if (GUILayout.Button("English (en)", GUILayout.Height(40))) { Application.OpenURL("https://melenitasdev.notion.site/melenitasdev/SOUNDS-GOOD-English-documentation-e2102ec5bafa411cb7991dba60081075"); }
        GUILayout.Space(1);
        if (GUILayout.Button("Spanish (es)", GUILayout.Height(40))) { Application.OpenURL("https://melenitasdev.notion.site/melenitasdev/SOUNDS-GOOD-Documentaci-n-espa-ol-b8fd14728ba74050a55a4c4f41ec0727"); }
    ...
    {
        GetStartedEditorWindow window = (GetStartedEditorWindow) GetWindow(typeof(GetStartedEditorWindow));
        window.Show();
    ...
    private void CreateDependencies()
    {
        ...
        if (mixer == null)
        {
            Debug.LogWarning($"Master Audio Mixer was not found. You must create " + $"it with the name 'Master' on the following path:\n{path}");
            allRight = false;
        }
